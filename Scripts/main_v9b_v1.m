...
tolerance=0.01;
%l=1;
proposedRnew=(referenceCase.blade.ispan(end)+hubRad);
    %while abs(proposedRnew-proposedR)>tolerance
    for l=1:6
        proposedR=proposedRnew;
        [crefjinputs,crefajam,crefapjam,crefjamchord,crefjamtwist,crefnewhubRad]...
                    =jamieson_v2_02_tipdfl(referenceCase,refmoment,wndspeed,fitcurve.a,...
                    fitcurve.n,fitcurve.p,hubRad,3,inputs,parentFolder,airfoils,proposedR);

        [crefjinputsph,crefajamph,crefapjamph,crefjamchordph,crefjamtwistph,crefnewhubRadph]...
                    =jamieson_v2_02_tipdfl(referenceCase,refmoment,wndspeed,fitcurve.a,...
                    fitcurve.n,fitcurve.p,hubRad,3,inputs,parentFolder,airfoils,proposedR+step);
        
        [crefjinputsmh,crefajammh,crefapjammh,crefjamchordmh,crefjamtwistmh,crefnewhubRadmh]...
                    =jamieson_v2_02_tipdfl(referenceCase,refmoment,wndspeed,fitcurve.a,...
                    fitcurve.n,fitcurve.p,hubRad,3,inputs,parentFolder,airfoils,proposedR-step);
        ...
        %topSecProps=struct;
                ...
                %Dstory=struct;
                    ...
                    [~,~,~,~,~,~,~,~,~,~,...
                        ~,~,~,~,~,~,~,~,~,~,Dph,~,~,~,~]...
                        =analysis_blocks_v2_02...
                        (crefjinputsph,crefnewhubRadph,parentFolder,airfoils,numel);

                    [~,~,~,~,~,~,~,~,~,~,...
                        ~,~,~,~,~,~,~,~,~,~,Dmh,~,~,~,~]...
                        =analysis_blocks_v2_02...
                        (crefjinputsmh,crefnewhubRadmh,parentFolder,airfoils,numel);
                    %topSecProps(count).secprops=blade.secprops.data;
                    %ispans(count).ispan=blade.ispan;
                    %Dstory(count).D=D;

        dflprime=(Dph(2,end)+Dmh(2,end))/(2*step);
        proposedRnew=proposedR-D(2,end)/dflprime;
        %l=l+1;
    end
